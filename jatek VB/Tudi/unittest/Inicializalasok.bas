...
Public Sub InitAll()
' Path = CurDir
 Path = "c:\mystuff\jatek\tudi\unittest"
' CreateGameSettings
 LoadGameSettings
' CreateDefaultBrushSettings
 ...
 NrUnit = 1
 VUnit(1).KX = BrushWidth \ 2
 VUnit(1).KY = BrushHeight \ 2
 VUnit(1).CanMove = 1
 VUnit(1).px = 8
 VUnit(1).py = 8
 Dim frmnr(12) As Integer
 Dim i As Integer
 Dim s As String
 For i = 1 To 12
  frmnr(i) = 5
 Next
 s = Path + "\unit1.bmp"
 VAnimUnit(1).Init s, 12, frmnr, 100, 100

 F_Map.Cls
 RefreshMap
End Sub

Public Sub CalcUnitMovement(ik As Integer)
  ' meg kell hatarozzuk az ax,ay -t
  Dim tx As Single
  Dim ty As Single
  Dim dx As Single
  Dim dy As Single
  Dim Szog As Double
  Dim t As Single
  t = (VUnit(ik).VY - VUnit(ik).KY)
  If t <> 0 Then
    Szog = (VUnit(ik).VX - VUnit(ik).KX) / t
    Else: Szog = 1
  End If
  If (t < 0) Then
   VUnit(ik).Szog = 90 + Atn(Szog) * (180 / 3.14)
   Else: VUnit(ik).Szog = 270 + Atn(Szog) * (180 / 3.14)
  End If
'  MsgBox Szog
  If t = 0 Then VUnit(ik).Szog = 90
  dx = Abs(VUnit(ik).KX - VUnit(ik).VX)
  dy = Abs(VUnit(ik).KY - VUnit(ik).VY)
  If (dx > dy) Then
    tx = 1
    ty = dy / dx
    ElseIf (dx < dy) Then
       ...
       Else: tx = 1
             ty = 1
  End If
  If (VUnit(ik).KX < VUnit(ik).VX) Then VUnit(ik).Ax = tx * VUnit(1).px
  If (VUnit(ik).KX > VUnit(ik).VX) Then VUnit(ik).Ax = -(tx * VUnit(ik).px)
  If (VUnit(ik).KY < VUnit(ik).VY) Then VUnit(ik).Ay = ty * VUnit(ik).py
  If (VUnit(ik).KY > VUnit(ik).VY) Then VUnit(ik).Ay = -(ty * VUnit(ik).py)
End Sub

Public Sub MoveUnit(ik As Integer)
  Dim i As Integer
  Dim j As Integer
  'a unit a kovetkezo poziciojara lepik vagy csak szimplan tolti az idot
  If (VUnit(ik).VX <> VUnit(ik).KX) Then
    i = (VUnit(ik).KX + VUnit(ik).Ax) \ BrushWidth + 1
    j = (VUnit(ik).KY + VUnit(ik).Ay) \ BrushHeight + 1
    If VBrush(MMap(j, i)).CanMove = 1 And VUnit(ik).CanMove = 1 Then
      VUnit(ik).KX = VUnit(ik).KX + VUnit(ik).Ax
      If (Abs(VUnit(ik).KX - VUnit(ik).VX) < Abs(VUnit(ik).Ax)) Then VUnit(ik).KX = VUnit(ik).VX
    End If
  End If
  If (VUnit(ik).VY <> VUnit(ik).KY) Then
    i = (VUnit(ik).KX + VUnit(ik).Ax) \ BrushWidth + 1
    j = (VUnit(ik).KY + VUnit(ik).Ay) \ BrushHeight + 1
    If VBrush(MMap(j, i)).CanMove = 1 And VUnit(ik).CanMove = 1 Then
      'lepjunk
      VUnit(ik).KY = VUnit(ik).KY + VUnit(ik).Ay
      'ha nem tud a pont megfelelo helyre lepni akkor segitsunk rajta
      If (Abs(VUnit(ik).KY - VUnit(ik).VY) < Abs(VUnit(ik).Ay)) Then VUnit(ik).KY = VUnit(ik).VY
    End If
  End If
'    Else 'ha nem kell lepjen akkor random pozicioba all
'          VUnit(ik).Anim = MaxUnitPoz
'  End If
End Sub
...
'          kep.PrintFrame 2, 1, Int(VUnit(1).KX - cx1 - BrushWidth / 2), Int(VUnit(1).KY - cy1 - BrushHeight / 2)
        If NrUnit <> 0 Then
          If (Abs(VUnit(1).Szog - 0) <= 15 Or VUnit(1).Szog > 345) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1), Int(VUnit(1).KY - cy1 - BrushHeight / 2), 1, 4
          If (Abs(VUnit(1).Szog - 30) <= 15) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1), Int(VUnit(1).KY - cy1), 1, 5
          If (Abs(VUnit(1).Szog - 60) <= 15) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1), Int(VUnit(1).KY - cy1), 1, 6
          If (Abs(VUnit(1).Szog - 90) <= 15) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1), Int(VUnit(1).KY - cy1), 1, 7
          If (Abs(VUnit(1).Szog - 120) <= 15) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1), Int(VUnit(1).KY - cy1), 1, 8
          If (Abs(VUnit(1).Szog - 150) <= 15) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1), Int(VUnit(1).KY - cy1), 1, 9
          If (Abs(VUnit(1).Szog - 180) <= 15) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1), Int(VUnit(1).KY - cy1), 1, 10
          If (Abs(VUnit(1).Szog - 210) <= 15) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1), Int(VUnit(1).KY - cy1), 1, 11
          If (Abs(VUnit(1).Szog - 240) <= 15) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1), Int(VUnit(1).KY - cy1), 1, 12
          If (Abs(VUnit(1).Szog - 270) <= 15) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1), Int(VUnit(1).KY - cy1), 1, 1
          If (Abs(VUnit(1).Szog - 300) <= 15) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1), Int(VUnit(1).KY - cy1), 1, 2
          If (Abs(VUnit(1).Szog - 330) <= 15) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1), Int(VUnit(1).KY - cy1), 1, 3
'          VAnimUnit(1).putpic Int(VUnit(1).KX - cx1 - BrushWidth / 2), Int(VUnit(1).KY - cy1 - BrushHeight / 2), 1, 12
          
'          If (VUnit(1).Ax = 0 And VUnit(1).Ay = 0) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1 - BrushWidth / 2), Int(VUnit(1).KY - cy1 - BrushHeight / 2), 1, 1
'          If (VUnit(1).Ax > 0 And VUnit(1).Ay > 0) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1 - BrushWidth / 2), Int(VUnit(1).KY - cy1 - BrushHeight / 2), 1, 2
'          If (VUnit(1).Ax > 0 And VUnit(1).Ay = 0) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1 - BrushWidth / 2), Int(VUnit(1).KY - cy1 - BrushHeight / 2), 1, 4
'          If (VUnit(1).Ax > 0 And VUnit(1).Ay < 0) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1 - BrushWidth / 2), Int(VUnit(1).KY - cy1 - BrushHeight / 2), 1, 5
'          If (VUnit(1).Ax = 0 And VUnit(1).Ay < 0) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1 - BrushWidth / 2), Int(VUnit(1).KY - cy1 - BrushHeight / 2), 1, 7
'          If (VUnit(1).Ax < 0 And VUnit(1).Ay < 0) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1 - BrushWidth / 2), Int(VUnit(1).KY - cy1 - BrushHeight / 2), 1, 8
'          If (VUnit(1).Ax < 0 And VUnit(1).Ay = 0) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1 - BrushWidth / 2), Int(VUnit(1).KY - cy1 - BrushHeight / 2), 1, 10
'          If (VUnit(1).Ax < 0 And VUnit(1).Ay > 0) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1 - BrushWidth / 2), Int(VUnit(1).KY - cy1 - BrushHeight / 2), 1, 11

'          If (VUnit(1).Ax >= 0) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1 - BrushWidth / 2), Int(VUnit(1).KY - cy1 - BrushHeight / 2), 1, 4
'          If (VUnit(1).Ax < 0) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1 - BrushWidth / 2), Int(VUnit(1).KY - cy1 - BrushHeight / 2), 1, 10
'          If (VUnit(1).Ay >= 0) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1 - BrushWidth / 2), Int(VUnit(1).KY - cy1 - BrushHeight / 2), 1, 1
'          If (VUnit(1).Ay < 0) Then VAnimUnit(1).putpic Int(VUnit(1).KX - cx1 - BrushWidth / 2), Int(VUnit(1).KY - cy1 - BrushHeight / 2), 1, 7
        End If
'          If NrUnit <> 0 Then VAnimUnit(1).PrintFrame 1, 1, Int(VUnit(1).KX - cx1 - BrushWidth / 2), Int(VUnit(1).KY - cy1 - BrushHeight / 2)
          MoveUnit 1
...
Public Sub DoUnit(ik As Integer)
 'nezzuk meg kelle forogjon a unitnak
  If VUnit(ik).KSzog <> VUnit(ik).Szog Then
     VUnit(ik).KSzog = VUnit(ik).KSzog + VUnit(ik).Speed
     If (Abs(VUnit(ik).KSzog - VUnit(ik).Szog) < VUnit(ik).Speed) Then VUnit(ik).KSzog = VUnit(ik).Szog
  End If
'az animacio a kovetkezo poziciora lepjen
End Sub
