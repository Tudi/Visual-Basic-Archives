...
Private Sub LoadBrushImg()
  ...
  frmn(MaxBrush - 1) = 1
  frmn(MaxBrush) = 1
  kep.Init Path + "\settings\brushimg4.jpg", MaxBrush, frmn
  F_Map.Cls
  RefreshMap
End Sub
...
For i = cx1 \ BrushWidth + 1 To VisibleBrushWidth
 For j = cy1 \ BrushHeight + 1 To VisibleBrushHeight
    If MMap(j, i) = 0 Then
...
Sub DrawMapGrid()
Dim i As Integer
backbuffer.SetForeColor (QBColor(13))
